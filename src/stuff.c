...
*/
		...
		if ((fdd = open(fdat, O_RDWR)) > 0)
		{
			...
			{
				if (mynote.date - notetmp.date < 14400)		/* 14400秒 */
					write(fdt, &notetmp, sizeof(notetmp));	/* 把舊檔兜進來 */
			}
			flock(fdd, LOCK_UN);
			close(fdd);
		}
...
int Goodbye()			/* quit bbs */
{
	...
	if (uinfo.ever_delete_mail)	/* 避免使用者不正常斷線導致信箱未清 */
...
/* By kmwang:20000529:For KHBBS */
	...
	{
		move(b_line, 24);
		outs("將線上訊息備份至信箱 (y/n) ? [y]: ");
		if (igetkey() != 'n')   /* sarek:06/09/2002:default to backup,避免user手滑漏存重要訊息... */
		{
			char fname[PATHLEN];
...
/*******************************************************************
 * 傳入 link list 的頂點, 釋放整個 Link list space
...
/*******************************************************************
 * 比對 link list 中是否有與 word 相同的資料.
 * --del by lasehu若 wcur 為 NULL, 則從頭找起, 否則從 wcur 開始找--
...
/*******************************************************************
 * 比對 link list 中是否有與 word 相同的資料.
 * 若 wcur 為 NULL, 則從頭找起, 否則從 wcur 開始找.
 * 如果有, 順便把該 link data 刪?
 * 假如 link data 是純指標, 則 freefunc 必須傳 NULL 進來
