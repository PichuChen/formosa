...
static void dochatcommand(char *cmd)
{
	...
	if (*cmd == 'h')
	{
		...
		printchatline("    /leave                  - 離開本頻道            [/le]");
		if (curuser.userlevel >= PERM_CLOAK)
			printchatline("    /cloak                  - 隱身                  [/cl]");
		...
		if (ch == I_OTHERDATA)
		{
			...
			if (status != 0)
			{
				...
				else if (status == 332)
				{
					...
					if ((p = strchr(pargv[3], ':')) != NULL)
					{
						sprintf(genbuf, "*** 本頻道的討論主題為: %s", p+1);
						printchatline(genbuf);
					}
				}
				/*
			...
			{
				sprintf(genbuf, "*** %s 禁止 %s 講話!", pargv[0], pargv[3]);
				printchatline(genbuf);
			}
...
/* :Kikeli!^riku@wopr.sci.fi MODE #sex -b+b *!hqghu@* *!*hqghu@* */
				...
				{
					sprintf(genbuf, "*** 解除 %s 頻道密碼!", pargv[2]);
					printchatline(genbuf);
				}
				else if (!strncmp(pargv[3], "+k", 2))
				{
					sprintf(genbuf, "*** 設定 %s 頻道密碼為: %s", pargv[2], pargv[3] + 3);
					printchatline(genbuf);
				}
			...
			{
				sprintf(genbuf, "*** %s 進來 %s 頻道囉! 大家好!", pargv[0], pargv[2]+1);
				printchatline(genbuf);
			}
			else if (!strcmp(pargv[1], "PART"))
			{
				sprintf(genbuf, "*** %s 離開 %s 頻道囉!", pargv[0], pargv[2]);
				printchatline(genbuf);
			}
#if 1
			else if (!strcmp(pargv[1], "INVITE"))
			{
				sprintf(genbuf, "*** %s 邀請您加入 %s 頻道!",
					pargv[0], pargv[3] + 1);
				printchatline(genbuf);
			}
#endif
			...
			{
				sprintf(genbuf, "*** %s 更換 %s 頻道主題為: %s",
					pargv[0], cur_chname, pargv[3] + 1);
				printchatline(genbuf);
			}
			else if (!strcmp(pargv[1], "QUIT"))
			{
				sprintf(genbuf, "*** %s 回家囉! 後會有期!", pargv[0]);
				printchatline(genbuf);
			}
			else if (!strcmp(pargv[1], "NICK"))
			{
				sprintf(genbuf, "*** %s 更換暱稱為: %s", pargv[0], pargv[2] + 1);
				printchatline(genbuf);
