	...
	if ( igetkey() == 'y' )
	{
		...
		if (isfile(ufname))
		{
			...
			/*
					...
					NULL, title, NULL);
			 */
			write_article_header(tmpfile, curuser.userid, uinfo.username, NULL, NULL, "[備份] 簽名檔", NULL);
			fputs("\n", tmpfile);
			...
			SendMail(-1, fname, curuser.userid, curuser.userid, "[備份] 簽名檔", curuser.ident);
			unlink(fname);
		}
	}

	msg("要將名片檔備份到站上信箱嗎?(y/n) [n]:");
	if ( igetkey() == 'y' )
	{
		//if (mailbox_is_full(0)) /* lthuang */
		if (check_mail_num(0))
		...
		if (isfile(ufname))
		{
			...
			/*
					...
					NULL, title, NULL);
			 */
			write_article_header(tmpfile, curuser.userid, uinfo.username, NULL, NULL, "[備份] 名片檔", NULL);
			fputs("\n", tmpfile);
			...
			SendMail(-1, fname, curuser.userid, curuser.userid, "[備份] 名片檔", curuser.ident);
			unlink(fname);
		}
	}

	msg("要將好友名單備份到站上信箱嗎?(y/n) [n]:");
	if ( igetkey() == 'y' )
	{
		//if (mailbox_is_full(0)) /* lthuang */
		if (check_mail_num(0))
		...
		if (isfile(ufname))
		{
			...
			/*
					...
					NULL, title, NULL);
			 */
			write_article_header(tmpfile, curuser.userid, uinfo.username, NULL, NULL, "[備份] 好友名單", NULL);
			fputs("\n", tmpfile);
			...
			SendMail(-1, fname, curuser.userid, curuser.userid, "[備份] 好友名單", curuser.ident);
			unlink(fname);
		}
	}
	return C_FULL;
