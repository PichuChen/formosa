...
char *direct;
{
	...
	if (!getdata_str(b_line, 0, "[往後找作者]: ", sbuf, sizeof(sbuf), ECHONOSP,
		     sbuf))
	{
		return C_FOOT;
	}
...
int title_backward(int ent, FILEHEADER *finfo, char *direct)
{
	...
	if (!getdata_str(b_line, 0, "[往前找標題]: ", sbuf, sizeof(sbuf), XECHO,
		     sbuf))
	{
		return C_FOOT;
	}
...
int title_forward(int ent, FILEHEADER *finfo, char *direct)
{
	...
	if (!getdata_str(b_line, 0, "[往後找標題]: ", sbuf, sizeof(sbuf), XECHO,
		     sbuf))
	{
		return C_FOOT;
	}
...
#define color	MENU_TITLE_COLOR
	...
	if (in_note)
		prints("留言板：%-14.14s [m", CurBList->filename);
	else
...
static int delthread(int ent, FILEHEADER *finfo, char *direct)
{
	...
	getdata(5, 0, "刪除 1)相關標題 2)相同作者 文章 ? [1]: ", ans, 2, XECHO);
...
int Read()
{
	...
	for (;;)
	{
		 ...
		 *	目前in_board只是一個switch
		 *		TRUE: 閱讀看板
		 *		FALSE: 閱讀精華區
		 *	所以in_board = TRUE並不代表使用者正在看板中閱讀,
		 *	或是已經選擇了某看板
		 */
		else if (in_board)	/* 一般區 */
		{
			/*
			 * 從精華區的子目錄按了tab退出cursor_menu, 回到這個Loop
			 * 的時候順便把精華區的depth降到最上層
			 */
			nowdepth = 1;
		...
		if (strcmp(curuser.userid, GUEST))	/* debug */
		{
			...
			else if (!in_note) /* 目前一般看板才有BM_ASSISTANT */
			{
